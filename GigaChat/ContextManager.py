...
class ContextManager(Prompts):
    ...
    def get_user_prompt(self):
        return """Я хочу составить туристический маршрут по местам, которые я перечислю ниже. Пожалуйста, учти их расположение, время на посещение и возможные интересные мероприятия или достопримечательности в каждом месте. Также постарайся сделать маршрут логичным и удобным для путешествия. Не забудь добавить ссылки и описания выбранных тобой мест. Они находятся в полях 'name' и 'url'. Выбери столько мест в день, сколько я попросил ранее. Вот список мест:\n"""
